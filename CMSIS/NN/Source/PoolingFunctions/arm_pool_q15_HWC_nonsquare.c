 ...
 * Title:        arm_pool_q15_HWC_nonsquare.c
...
static void compare_and_replace_if_larger_q15(q15_t *base,           // base data
                                             const q15_t *target,   // compare target
                                             const uint16_t length // data length
)
{
    q15_t *pIn = base;
    const q15_t *pCom = target;
    union arm_nnword in;
    ...
    while (cnt > 0u)
    {
        in.word = arm_nn_read_q15x2((const q15_t *)pIn);
        com.word = arm_nn_read_q15x2_ia((const q15_t **)&pCom);
...
/**
 * @brief Q15 max pooling function (non-square shape)
...
void arm_maxpool_q15_HWC_nonsquare(q15_t *Im_in,
                        ...
                        q15_t *Im_out)
{
    ...
    for (i_y = 0; i_y < dim_im_in_y; i_y++)
    {
        ...
        for (i_x = 0; i_x < dim_im_out_x; i_x++)
        {
            ...
            q15_t *target = Im_in + (i_y * dim_im_in_x + i_x) * ch_im_in;
            q15_t *win_start;
            q15_t *win_stop;
    ...
    for (i_y = 0; i_y < dim_im_out_y; i_y++)
    {
        ...
        /* for each output row */
        q15_t *target = Im_out + i_y * dim_im_out_y * ch_im_in;
        q15_t *row_start;
        q15_t *row_end;
        ...
        {
            compare_and_replace_if_larger_q15(target, row_start, dim_im_out_x * ch_im_in);
        }
    }
